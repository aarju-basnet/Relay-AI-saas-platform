...
import { MessageSquare, Send, Search, Crown, Shield, Copy, Trash2, Check } from "lucide-react";
...
function formatMessageDate(dateString: string): string {
  ...
  // For older years, outputs "04/09/2024 at 17:57"
  return `${date.toLocaleDateString()} at ${timeStr}`;
}

  return (
    <div className="flex h-full">
      {/* ── TEAM MEMBERS (LEFT PANEL) ── */}
      <div className="w-64 shrink-0 border-r border-border h-full overflow-y-auto hidden lg:block">
          ...
          {members.map((member) => (
            <div key={member.id} className="flex items-center gap-3 px-4 py-3">
              <MemberAvatar
                ...
                isLeadership={member.role === "OWNER" || member.role === "ADMIN"}
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-1 mt-0.5">
                  {member.role === "OWNER" && (
                    <Crown size={10} className="text-copper" />
                  )}
                  {member.role === "ADMIN" && (
      ...
      <div className="flex flex-col h-full flex-1 min-w-0">
        ...
        <div className="border-b border-border px-6 py-4 space-y-3">
          ...
          <div className="flex items-center gap-2 rounded-xl border border-border bg-canvas px-3 py-2 max-w-md">
...
function MemberAvatar({
  ...
  return (
    <div className="relative shrink-0">
      ...
      {!isLeadership && isActive && (
        <span
          title="Active account"
          className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-green-500 border-2 border-surface"
        />
      )}
    </div>
  );
