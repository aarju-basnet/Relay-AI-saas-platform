...
const { user, activeWorkspace, refreshUser, developerMode, setDeveloperMode } = useAuth();
  const [searchParams, setSearchParams] = useSearchParams();
  const [, setShowUpgradeBanner] = useState(false);
  ...
  const [, setAssistant] = useState<Assistant | null>(null);
  ...
  return (
    <div
      className={`flex flex-col h-screen bg-canvas text-ink ${
    ...
    >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Outer Column Wrapper for TopBar and Main content area */}
...
<div>
  <div className="flex items-center gap-2">
    ...
    <RoleBadge role={activeWorkspace?.role} />
                ...
                <div className="rounded-2xl border border-border bg-surface shadow-sm overflow-hidden">
                  ...
                  <div className="flex flex-col items-center justify-center py-10 text-center">
                    ...
                    <p className="text-[11px] text-ink-muted mt-1 max-w-xs">
                      Connect your own OpenAI, Anthropic or Google API key to
            ...
            {/* ── SETTINGS ──────────────────────────────────── */}
            {page === "settings" && <Settings settingsPage={settingsPage} />}

            {/* ── DEVELOPER ─────────────────────────────────── */}
            {page === "developer" && <DeveloperDashboard />}
...
function NewDeviceAlertBanner({ onEnable2FA }: { onEnable2FA: () => void }) {
  ...
  useEffect(() => {
    api
      ...
      .then((res) => {
        setMuted(res.muted);
        const latestUnread = res.notifications.find((n) => !n.read);
        setNotification(latestUnread ?? null);
      })
      .catch(() => {});
  }, []);

  if (dismissed || muted || !notification) return null;

  const isSecurity = notification.type === "NEW_DEVICE_LOGIN";

  return (
    <div
      className={`border-b px-6 py-2.5 flex items-center justify-between text-sm ${
        isSecurity
          ? "bg-red-50 border-red-200"
          : "bg-copper-dim border-copper/20"
      }`}
    >
      <span className="text-ink-muted flex items-center gap-2">
        {isSecurity ? (
          ...
          <Bell size={15} className="text-copper shrink-0" />
        )}
      ...
      <div className="flex items-center gap-4 shrink-0 ml-4">
        {isSecurity && (
          <button
            onClick={onEnable2FA}
            className="text-copper hover:text-copper-bright font-medium"
          >
            Enable 2FA
          </button>
        )}
